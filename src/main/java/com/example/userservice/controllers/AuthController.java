...
import com.example.userservice.models.SessionUser;
...
import com.example.userservice.payload.response.TokenRefreshResponse;
import com.example.userservice.repositories.RoleRepository;
import com.example.userservice.repositories.UserRepository;
import com.example.userservice.security.jwt.JwtUtils;
import com.example.userservice.services.RedisService;
...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
        ...
        Date date = new Date();
        String jwt = jwtUtils.generateJwtToken(authentication, date);
        UserDetailsImpl userDetails = (UserDetailsImpl) authentication.getPrincipal();
        List<String> roles = userDetails.getAuthorities().stream()
                .map(item -> item.getAuthority())
                .collect(Collectors.toList());
        RefreshToken refreshToken = accountService.createRefreshToken(userDetails.getId());
        SessionUser sessionUser = new SessionUser(userDetails.getUsername(), date, true);
        redisService.setValue(redisHashConvert.getKeySessionKey(userDetails.getUsername(), date), sessionUser, TimeUnit.SECONDS, 60L, true);
    ...
    public ResponseEntity<?> refreshtoken(@RequestBody TokenRefreshRequest request) {
        ...
        if ((refreshToken != null) && (refreshToken.length() > 0)) {
            return accountService.findByRefreshToken(refreshToken)
                    .map(accountService::verifyExpiration)
                    .map(RefreshToken::getUserId)
                    .map(userId -> {
                        User user = userRepository.findById(userId).orElseThrow(()
                                -> new UserNotFoundException("User is not existed"));
                        TokenRefreshResponse tokenRefreshResponse = new TokenRefreshResponse(jwtUtils.generateTokenFromUsername(user.getUsername()), refreshToken);
